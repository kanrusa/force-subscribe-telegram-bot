    ...
    if chat_member.restricted_by:
      if chat_member.restricted_by.id == (client.get_me()).id:
          try:
            ...
            client.answer_callback_query(cb.id, text="❗ Silahkan join 'channel' dan tekan tombol 'UnMute Me'.", show_alert=True)
      else:
        client.answer_callback_query(cb.id, text="❗ Kamu di mute karena alasan lain.", show_alert=True)
    else:
      if not client.get_chat_member(chat_id, (client.get_me()).id).status == 'administrator':
        ...
        client.answer_callback_query(cb.id, text="❗ Warning: Jangan klik kalau kamu tidak sedang di mute.", show_alert=True)
      ...
      try:
        ...
        try:
          sent_message = message.reply_text(
              "{}, kamu **belum subscribe**  [channel bjrp rated](https://t.me/{}). silahkan [join channel](https://t.me/{}) dan setelah itu **tekan tombol unmute dibawah** agar kamu bisa mengirim pesan di grup.".format(message.from_user.mention, channel, channel),
              disable_web_page_preview=True,
              reply_markup=InlineKeyboardMarkup(
                  [[InlineKeyboardButton("UnMute Saya", callback_data="onUnMuteRequest")]]
              )
          )
          client.restrict_chat_member(chat_id, user_id, ChatPermissions(can_send_messages=False))
        except ChatAdminRequired:
          sent_message.edit("❗ **Saya bukan admin.**\n__Jadikan saya admin dengan ban permision dan tambahkan saya lagi.\n#Leaving this chat...__")
          client.leave_chat(chat_id)
      except ChatAdminRequired:
        client.send_message(chat_id, text=f"❗ **Saya bukan admin di channel @{channel}**\n__jadikan saya admin dan tambahkan saya lagi.\n#Leaving this chat...__")
        client.leave_chat(chat_id)
